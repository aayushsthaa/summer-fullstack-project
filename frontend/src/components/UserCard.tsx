...
const UserCard = ({ user }: { user: IUser }) => {
  ...
  const bioSnippet = profile?.bio ? (profile.bio.length > 80 ? `${profile.bio.substring(0, 80)}...` : profile.bio) : 'No bio available.';
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700 p-6 flex flex-col h-full hover:shadow-xl hover:border-blue-500 dark:hover:border-blue-400 transition-all duration-300">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-grow truncate">
          <h3 className="text-lg font-bold text-gray-800 dark:text-white truncate">{name}</h3>
        ...
        <div className="flex flex-wrap gap-2 min-h-[30px]">
          {topSkills.length > 0 ? (
            topSkills.map((skill, index) => (
              <span key={index} className={`px-2 py-1 text-xs font-medium rounded-full ${getSkillBadgeColor(skill.level)}`}>
