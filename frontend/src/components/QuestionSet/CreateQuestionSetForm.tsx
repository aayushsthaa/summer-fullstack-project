  ...
  const onSubmitHandler = (data: QuestionSetForm) => {
    ...
    for (const q of data.questions) {
        if (!q.questionText.trim()) {
            setModal({ isOpen: true, title: "Validation Error", message: "Please ensure all questions have text.", type: "error"});
            return;
        }
        if (q.choices.length === 0) {
            setModal({ isOpen: true, title: "Validation Error", message: `A question must have at least one choice.`, type: "error"});
            return;
        }
        if (!q.choices.some(c => c.correctAnswer)) {
            setModal({ isOpen: true, title: "Validation Error", message: `A question must have at least one correct answer.`, type: "error"});
            return;
        }
        for (const c of q.choices) {
            if (!c.text.trim()) {
                setModal({ isOpen: true, title: "Validation Error", message: `Please ensure all choices have text.`, type: "error"});
                return;
            }
        }
    }
...
function CreateQuestions() {
  ...
  return (
    <div className="space-y-10">
      <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Questions</h2>
      {fields.map((field, index) => (
        <div key={field.id} className="border-t border-gray-200 dark:border-gray-600 pt-6">
          <div className="flex justify-between items-start mb-4">
            <label className="block text-lg font-semibold text-gray-700 dark:text-gray-200">Question {index + 1}</label>
            {fields.length > 1 && (
              <button
                type="button"
...
function CreateChoices({ questionIndex }: { questionIndex: number }) {
  ...
  return (
    <div className="space-y-4">
        ...
        <h4 className="font-semibold text-gray-600 dark:text-gray-300 mt-4">Choices</h4>
        <div className="space-y-3 pl-2">
            ...
            <div key={field.id} className="flex items-center gap-3">
                ...
                />
                <input
                    {...register(`questions.${questionIndex}.choices.${index}.text`, { required: true })}
