        ...
        <FormProvider {...methods}>
          <form onSubmit={handleSubmit(onSubmitHandler)} className="space-y-8">
            <div className="text-center">
              ...
              <p className="mt-2 text-lg text-gray-500 dark:text-gray-400">Answer all questions to the best of your ability.</p>
            </div>
  ...
  return (
    <div className="space-y-10">
      {fields?.map((field, index) => {
        return (
          <div key={field.id} className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-2xl shadow-md border border-gray-200 dark:border-gray-700">
            <h3 className="text-xl font-semibold text-gray-800 dark:text-white mb-6">
              <span className="text-blue-600 dark:text-blue-400">Q{index + 1}:</span> {field.questionText}
            </h3>
        ...
        return (
          ...
          >
            <input
              ...
              className="hidden"
            />
            <span className={`font-semibold ${isSelected ? "text-white" : "text-gray-800 dark:text-gray-200"}`}>{field.text}</span>
          </label>
        );
      })}
    </div>
